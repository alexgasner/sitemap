...
export default function LayerControls({ viewMode, season, onViewModeChange, onSeasonChange }: LayerControlsProps) {
  ...
  // Hide season toggle for modes where it's irrelevant
  const showSeasons = viewMode !== "base" && viewMode !== "microzones";

  return (
    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 z-20 print:hidden">
      ...
      {/* Main Layer Toggle */}
      <div className={clsx(
        "bg-background/95 backdrop-blur-md border border-border/80 shadow-lg rounded-xl p-1.5 flex items-center gap-1",
        ...
        {modes.map((mode) => (
          <button
            ...
            className={clsx(
              "flex flex-col items-center gap-1.5 rounded-lg transition-all duration-200",
              isMobile ? "px-2 py-2 min-w-[40px]" : "px-4 py-2.5 min-w-[80px]",
              viewMode === mode.id
